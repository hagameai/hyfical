...
class GitOperations:
    ...
    def commit_changes(repo_name, message):
        """
        Commits changes in the specified Git repository with a given message.
    ...
    def status(repo_name):
        """
        Shows the status of the specified Git repository.

        :param repo_name: Name of the repository to check the status.
        :return: None.
        """
        try:
            subprocess.check_call(['git', 'status'], cwd=repo_name)
        except Exception as e:
